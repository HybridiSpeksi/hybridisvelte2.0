<script>
    ...
    let biisit = data.produktioData.items[0].biisitCollection.items
    console.log(biisit)
    // biisit = data.body.pageData.items[0].fields.biisit
...
{#each biisit as biisi}
<h2>{biisi.nimi}</h2>
<h3><em>{biisi.origin}</em></h3>
<div>{@html documentToHtmlString(biisi.sanat.json)}</div>
{/each}
